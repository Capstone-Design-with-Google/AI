...
from config import GCP_SERVICE_ACCOUNT_KEY_PATH, AUDIO_CLIPS_FOLDER, TTS_LANGUAGE_CODE, TTS_VOICE_NAME_NEURAL
...
def synthesize_text_to_speech(text_to_synthesize, output_filename, scene_number):
    try:
        ...
        print(f"  Synthesizing speech for scene {scene_number}: '{text_to_synthesize[:30]}...'")
...
def generate_audio_clips_from_scenario(scenario_data, product_name):
    ...
    for scene in scenario_data:
        ...
        updated_scenario_data.append(scene)
    print("음성 클립 생성 완료.")
